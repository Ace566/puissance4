...
class GameBoard {
  List<List<Color>> _boxes = List.generate(7, (i) => List.generate(6,(i) => null),);

  GameBoard();

  GameBoard.from(List<List<Color>> boxes) {
    _boxes = boxes;
  }
  ...
  void reset() {
    ...
    _boxes = new List.generate(7,(i) => List.generate(6, (i) => null));
  ...
  GameBoard clone() {
    return GameBoard.from(_boxes.map((c) => c.map((b) => b).toList()).toList());
  }
}
